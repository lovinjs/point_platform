...
- 用户可以在任意合作门店消费通用积分。
- 消费门店按照消费金额参与平台结算。
- 平台手续费默认 5%，门店默认获得 95%；超级管理员可按合作政策调整新订单费率。
...
- 对于重复扣减、操作错误等异常情况，超级管理员可以发起冲正。冲正不是普通退款，必须单独记录原因和操作信息，不需要二次审批；该流程现已实现。
...
消费积分 = P
消费金额（分） = P × 100
平台手续费 = 消费金额 × 订单创建时的平台费率
门店应结算金额 = 消费金额 - 平台手续费
~~~

例如平台费率为默认的 5%，消费 100 个积分：
...
平台费率由超级管理员在业务参数中维护，范围为 0% 到 100%，修改原因和修改前后快照必须进入审计日志。新费率只用于保存后新创建的消费订单；已经创建的待确认订单和历史订单继续使用自身快照。V1 的兑换比例固定为 1 元 = 1 积分，赠送积分固定关闭，不允许通过后台配置修改。
...
#### platform_business_setting

V1 只保存一行全局业务参数：

- id，固定为 1
- platform_fee_rate_bps
- consumption_pending_ttl_minutes
- version
- last_updated_by
- create_time
- update_time

version 用于防止两个后台页面同时保存时互相覆盖。消费订单创建时读取当前配置，并把费率、手续费、门店应付金额和过期时间保存到订单快照中。
...
为了避免多个订单混淆，V1 同一客户同时只能存在一个 PENDING_CONFIRM 消费订单。待确认订单默认有效期为 5 分钟，超级管理员可在 1 至 30 分钟范围内调整；调整只影响之后创建的订单。
